...
void stat() {
  ...
  int barWidth = 80;
  float progress =
      static_cast<float>(median - min_measured) / (max_measured - min_measured);
...
TickTwo stat_ticker([]() { stat(); }, 20, 0, MILLIS);
TickTwo arduino_ota_ticker([]() { ArduinoOTA.handle(); }, 1, 0, MILLIS);
...
void setup() {
  ...
  const char *ssid = "kakaoscsiga";
  const char *password = "Ladybug7";
